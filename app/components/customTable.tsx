...
//         showLoading?: boolean;
//         label?: string;
//         onClick?: (data: TableDataType, params?: string) => void;
//     }[];
//     table?: {
//         width?: number | string;
...
//         // Now handled by query params in FilterBy component
//     }, [columnsSignature]);
...
//             const initialPage = searchParams ? parseInt(searchParams.get('page') || '1', 10) - 1 : 0;
//             const pageNoForApi = initialPage + 1;
//             const pageSize = config.pageSize || defaultPageSize;
//             // Only call API if params changed
//             if (
//                 !lastApiCallRef.current ||
//                 lastApiCallRef.current.pageNo !== initialPage ||
//                 lastApiCallRef.current.pageSize !== pageSize
//             ) {
//                 lastApiCallRef.current = { pageNo: initialPage, pageSize };
//                 try {
//                     setNestedLoading(true);
//                     const result = await config.api.list(pageNoForApi, pageSize);
//                     const resolvedResult = result instanceof Promise ? await result : result;
//                     const { data, total, currentPage } = resolvedResult;
//                     const tableInit = {
//                         data,
...
//                                                                         setIsLoading(true);
//                                                                         try {
//                                                                             // Ensure the current history entry contains the full query params
//                                                                             // so that browser Back returns to the filtered state.
...
//                                                                             // Call the provided handler. We pass current params as second arg
//                                                                             // for handlers that accept it, but we don't rely on them.
//                                                                             await action.onClick(row, params.toString());
//                                                                         } finally {
//                                                                             setIsLoading(false);
//                                                                         }
//                                                                     }
...
//                     } catch (err) { }
//                     // Update query params: remove this column filter and reset to page 1
//                     try {
//                         const params = new URLSearchParams(window.location.search);
...
//                     } catch (err) { }
//                     // Update query params: set this column filter and reset to page 1
//                     try {
//                         const params = new URLSearchParams(window.location.search);
...
//                 }
//             }
//             // If no external onSelect provided, handle selection internally
//             if (!filterConfig?.onSelect) {
//                 const currentlySelected = selectedRef.current as string | null;
//                 const newVal = currentlySelected === value ? "" : value;
//                 // update refs/state
//                 selectedRef.current = newVal;
//                 setSelectedValues(newVal ? [newVal] : []);
//                 try {
//                     const payload = { ...(filterState?.payload || {}), [column]: newVal } as Record<string, any>;
//                     if (!newVal) delete payload[column];
//                     setFilterState({ applied: Object.keys(payload).length > 0, payload });
//                 } catch (err) { }
//                 // update URL
//                 try {
//                     const params = new URLSearchParams(window.location.search);
//                     if (newVal) params.set(column, newVal); else params.delete(column);
//                     params.set('page', '1');
//                     router.replace(window.location.pathname + (params.toString() ? '?' + params.toString() : ''), { scroll: false });
//                 } catch (err) { }
...
//                 try {
//                     setNestedLoading(true);
//                     const payloadForApi: Record<string, any> = {};
//                     if (newVal) payloadForApi[column] = newVal;
//                     if (api?.filterBy) {
//                         const res = await api.filterBy(payloadForApi, defaultPageSize, 1);
//                         const result = res instanceof Promise ? await res : res;
//                         const { data, total, currentPage } = result;
//                         setTableDetails({ data, total: total || 1, currentPage: (currentPage || 1) - 1, pageSize: defaultPageSize });
//                     } else if (api?.search) {
//                         const res = await api.search(newVal || '', defaultPageSize, column, 1);
//                         const result = res instanceof Promise ? await res : res;
//                         const { data, total, currentPage } = result;
//                         setTableDetails({ data, total: total || 1, currentPage: (currentPage || 1) - 1, pageSize: defaultPageSize });
//                     } else if (api?.list) {
...
//                         setTableDetails({ data, total: total || 1, currentPage: (currentPage || 1) - 1, pageSize: defaultPageSize });
//                     }
//                 } catch (err) { /* ignore */ } finally { setNestedLoading(false); }
//             }
...
//                 // setNestedLoading(true);
//                 const payloadForApi: Record<string, any> = {};
//                 if (updated.length > 0) payloadForApi[column] = updated;
//                 if (api?.filterBy) {
//                     const res = await api.filterBy(payloadForApi, defaultPageSize, 1);
//                     const result = res instanceof Promise ? await res : res;
//                     const { data, total, currentPage } = result;
//                     setTableDetails({ data, total: total || 1, currentPage: (currentPage || 1) - 1, pageSize: defaultPageSize });
//                 } else if (api?.search) {
//                     const res = await api.search(updated.join(','), defaultPageSize, column, 1);
//                     const result = res instanceof Promise ? await res : res;
//                     const { data, total, currentPage } = result;
//                     setTableDetails({ data, total: total || 1, currentPage: (currentPage || 1) - 1, pageSize: defaultPageSize });
//                 } else if (api?.list) {
...
//                 } catch (err) {
//                     console.error('Failed to parse custom filters', err);
//                     return {};
//                 }
//             }
//         }
...
//     };
//     const applyFilter = useCallback(async (overridingFilters?: Record<string, any>, pageNo?: number) => {
...
//     // Update query params with filter values and set page
//     const params = new URLSearchParams(window.location.search);
...
//         }
//     });
//     // Set page to provided pageNo or 0
//     const pageToSet = pageNo !== undefined ? pageNo : 0;
//     params.set('page', String(pageToSet + 1));
//     // FIX: get router from useRouter
//     const router = useRouter();
//     router.replace(window.location.pathname + '?' + params.toString());
...
//             const res = await config.api.filterBy(payloadForApi, config.pageSize || defaultPageSize, pageToSet + 1);
...
//                     const field = fields.find(f => f.key === k);
//                     try {
//                         if (field?.applyWhen && !field.applyWhen(currentFilters)) {
//                             return true;
//                         }
//                     } catch (err) {
//                         return true;
...
// const applyCustomPayload = useCallback(async (payload?: Record<string, any>, pageNo?: number) => {
...
//             const params = new URLSearchParams(window.location.search);
//             Object.keys(effectivePayload || {}).forEach((k) => {
//                 const v = effectivePayload[k];
//                 if (Array.isArray(v)) {
//                     if (v.length > 0) {
//                         params.set(k, v.join(','));
//                     } else {
...
//         const params = new URLSearchParams(window.location.search);
//         Object.keys(cleared).forEach((k) => params.delete(k));
...
//         // Update query params: remove filters and reset page
